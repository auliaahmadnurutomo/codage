...
class SettingUsersController extends Codeton
{
    ...
    public function __construct(Request $request, Logger $logger)
    {
        ...
        View::share([
            ...
            'offices' => DB::table('skeleton_setting_office')->where('status', 1)->get(),
            'staff_position' => DB::table('skeleton_setting_position')->where('status', 1)->get(),
            'department' => DB::table('skeleton_setting_department')->where('status', 1)->get(),
            'roles' => DB::table('skeleton_setting_menu_template')->where('status', 1)->get()
        ]);
    }
    ...
    public function Update(Request $request)
    {
        ...
        try {
            ...
            //log jika error
            $this->logger->error($e);
            return $this->Return_response('Error during update '.$e, 'response/error_reload_js');
        }
        //log kalau sukses
